...
#ifdef LAB_PGTBL
void vmprint_helper(pagetable_t pagetable, int level, uint64 parent_va) {
  // base case
  if (level < 0)
      return;
  // recursive case
  for (int i = 0; i < 512; ++i) {
      pte_t *pte = &pagetable[i];
      if (*pte & PTE_V) {
          uint64 va = parent_va + i * 4096;
          for (int j = level; j <= 2; ++j) {
              printf(".. ");
          }
          printf("%p: pte %p pa %p\n", (uint64*)va, pte, (uint64*)PTE2PA(*pte));
          pagetable_t next_pagetable = (pagetable_t)PTE2PA(*pte);
          vmprint_helper(next_pagetable, level - 1, va);
      }
  }
}
void
